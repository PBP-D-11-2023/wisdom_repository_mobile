...
class _PeminjamanPageState extends State<PeminjamanPage> {
  Future<List<Peminjaman>> fetchPeminjaman(CookieRequest request) async {
    var response = await request
        .get('http://localhost:8000/borrow/peminjamanjson/');
                ...
                if (!snapshot.hasData) {
                  ...
                  return ListView(
                    children: [
                      ...
                      Padding(
                        padding: const EdgeInsets.all(10.0), // Adjust the values as needed
                        child: Text(
                          'List of books:',
                                        ...
                                        ElevatedButton(
                                          onPressed: () async {
                                           final response = await request.postJson(
                                                "http://localhost:8000/borrow/pengembalianflutter/",
                                                jsonEncode(<String, String>{
                                                  'idPeminjaman': snapshot.data![index].pk.toString(),
                                                  'idBuku': snapshot.data![index].idBuku.toString(),
                                                }));
                                            if (response['status'] ==
                                                'success') {
